...
| `Sidebar.js` | Right-hand panel UI. Shows ONLY the selected node's/edge's own fields (per `NodeOptions`) when something is selected - `#sidebar` gets an `editing="node"/"edge"` attribute (tinting the background) via a wrapped `showPage`. The Node page also offers a "split node" button (an explicit, click-based alternative to `Dragger.js`'s Alt+drag split gesture - same `Model.splitNode` underneath), shown only when Merge & Split is on. When nothing is selected (the "Edit" page, reached by clicking empty canvas space - the app's title lives in the floating `#loopy_header` widget instead, see `Loopy.js`), it shows, in order: Layout (auto-arrange buttons), the Node Groups editor, a collapsible "Model options" section (Node/Edge field toggles), and a collapsible "Save, share or load" section (history manager, export/import, embed, clear graph). Built from `ComponentSlider`, `ComponentInput`, `ComponentButton`, `ComponentNodeGroup`, `ComponentNodeType`, `ComponentEdgeType`, `ComponentCluster`, `ComponentClusterDescription` components. |
...
| `Dragger.js` | Move tool — drags nodes/labels; also supports rectangular multi-select. Dropping one node onto another offers to merge them (`Model.mergeNodes`, with a confirm dialog) — the merged node keeps a Node Group only if both sides agreed on one (otherwise ungrouped), and concatenates both sides' Descriptions as `"NodeA: ...\nNodeB: ..."` (a side with no description contributes no line). Holding Alt/Option while starting to drag a node instead peels off a full duplicate ("twin", spawned on the first `mousemove` via `Model.splitNode`; also reachable via a "split node" button on the Node sidebar page, see `Sidebar.js`) that the rest of the gesture drags around like any other node — including being droppable onto a third node to merge there. Both merge and split are gated behind the "Merge & Split" toggle (`NodeOptions.js`'s `mergeSplit`, off by default); when off, dropping a node on another just leaves them overlapping, and Alt+drag behaves like an ordinary drag. A split duplicates every edge touching the original onto the twin and downgrades ALL of them (the original's own edges too) to `edgeType: "questionable"`, since neither node's inherited connections are "confirmed" until reviewed. |
...
| `NodeGroups.js` | Diagram-level list of 1-8 named colour groups (a node's `hue` is an index into this list), using the fixed, colour-blind-safe Okabe-Ito palette — group *N* always uses palette colour *N*; only a group's name and the group count are editable. A node can also be explicitly ungrouped (`hue === null`), rendered in a fixed grey (`NULL_COLOUR`) — the "clear node group" swatch in the sidebar sets this, and it's also what a node reverts to if its group is removed via `removeLastGroup()`. Publishes `groups/changed` on add/remove/load; `Model.js` listens to rebuild `COLOUR_NODE_LIST`. When `NodeOptions`'s `clusterByGroups` toggle is on, renaming a group here (`setName`) renames its members' Cluster too, via `Clusters.renameCluster` (see `Loopy.js`'s `syncClustersToGroups`). |
| `Clusters.js` | Optional, free-text node attribute (`node.cluster`, "" = none, off by default via `NodeOptions`'s `cluster` key) — unlike Node Groups (a fixed 1-8 slot palette), any number of clusters can exist, named however typed, and membership is just that string on the node. A cluster's DESCRIPTION lives here instead, in a diagram-level name→description registry (`self.registry`), shared by every node with that same cluster name and looked up live by name (`getDescription`/`setDescription`) - never cached per-node. Two distinct operations Sidebar.js's `ComponentCluster` offers on a node's cluster: REASSIGN (typing/picking a name in the Cluster field just edits `node.cluster` directly - moves THIS node to a possibly-new cluster) vs RENAME (`renameCluster(oldName, newName)`, via the field's "rename" button - retags every node currently in `oldName` to `newName` and carries the description over, merging into `newName`'s existing description if it already names a distinct cluster). Registry entries aren't pruned when a cluster loses its last member. A "Cluster by groups" suboption (Sidebar.js, under the Cluster checkbox, gated by `NodeOptions`'s `clusterByGroups` key) keeps every node's cluster identical to its Node Group's name for as long as it's on — see `Loopy.js`'s `syncClustersToGroups`; the description this drives still lives here, keyed by that shared name, never on `NodeGroups.js`. |
...
```
[nodes[], edges[], labels[], settings[], UID, groupNames[], clusterEntries[], modelInfo]
```

Node fields (by index): `id, x, y, init, label(encoded), hue, radius, gain, strength, active, topLabel(encoded), bottomLabel(encoded), description(encoded), cluster(encoded)`

Edge fields: `fromId, toId, arc, strength, speedMultiplier, [rotation], edgeType`

`settings[]` holds `[MAX_SIGNAL_AGE, MAX_SIGNALS_PER_EDGE, MAX_SIGNALS, showType, showGroup, showStartAmount, showRadius, showGain, showStrength, showEdgePolarity, showAttenuation, showSpeed, showEdgeType, allowMergeSplit, showCluster, clusterByGroups]` — entries 3+ are the diagram-embedded defaults for `NodeOptions.js`'s toggles (6 node keys, then 4 edge keys, then the interaction key `mergeSplit`, then the second-wave node key `cluster`, then the third-wave node key `clusterByGroups`, each wave appended last so old links' indices don't shift) and may be absent/short in links saved by older versions (treated as "no diagram default" rather than an error).
