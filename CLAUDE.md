...
| `NodeOptions.js` | Global, per-field visibility toggles for the Node and Edge sidebar pages, shown in the Sidebar's collapsible "Model options" grouped as Node Fields (Size), Edge Fields (Edge Type, Edge Polarity), and Simulation (Node Type, Start Amount, Gain, Quantum, Signal Attenuation, Signal Speed) — all off by default; a Node's Name, Description, and Node Group are always shown (not toggleable). Also holds the non-field "Merge & Split" interaction toggle (`mergeSplit`, off by default, under Node Fields) that gates `Dragger.js`'s drag-node-onto-node merge. Merges a diagram-serialized default with a per-browser `localStorage` override and publishes `settings/changed` on change. "direction" (Edge Polarity) and "edgeType" (Edge Type) are special: "direction" picks between two mutually-exclusive canvas display modes for a "-" edge's polarity (colour vs +/- icon) rather than just showing/hiding a field, and "edgeType"'s visuals (and simulation effect) always apply regardless of its toggle — see `Edge.js`. |
